...
#
# It uses the package ndf instead of RNetCDF.
...
# Very early warning of next El Ni̱o, Proceedings of the 
            ...
            for (idx in 1:n.points) {
                    lon <- lon.from.index(idx)
                            lat <- lat.from.index(idx)
                                pointnames[idx] <- paste0("S", sprintf("%03d", lat), "E",
                                        sprintf("%03d", lon))
...
make.Kvals.for.year <- function(year) { 
      onc <- open.ncdf(paste0("air.sig995.", year, ".nc"))
      data <- get.var.ncdf(onc)
                yearof.Kvals <- matrix(0, nrow=365, ncol=n.points)
                  for (lat in lat.range) {
                          for (lon in lon.range) { 
                                    yearof.Kvals[, index.table[lat, lon] ] <- data[lon,lat,1:365] # ignore leap days
                                                }
                            }
                            close.ncdf(onc)
